...
have paths relative to something other than the current working directory, 
provide the `basedir` option to indicate the "root level" of the paths.
...
```json
{
    ...
    "browserify": {
        "transform": [
            ["reactify", {"basedir": "/home/whoever/whatever"}]
        ]
    }
}
```
